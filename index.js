...
async function startIndependentBot(num) {
    ...
    sock.ev.on('connection.update', async (update) => {
        ...
        if (connection === 'close') {
            ...
            console.log(`[Session ${num}] Fermée : ${reason} (${lastDisconnect?.error?.message || 'raison inconnue'})`);
            // Nettoyage de l'ancienne socket
            delete sessions[num];
            ...
            if (reason === DisconnectReason.loggedOut) {
                console.log(`[Session ${num}] Session expirée/déconnectée définitivement.`);
                setTimeout(() => {
                    if (fs.existsSync(specificDir)) fs.rmSync(specificDir, { recursive: true, force: true });
                }, 3000);
            } else if (reason === DisconnectReason.connectionReplaced) {
                console.log(`[Session ${num}] Connexion remplacée par une autre session. Pas de reconnexion.`);
            } else {
                // Reconnexion avec délai exponentiel selon la raison
                let delay = 5000;
                if (reason === DisconnectReason.connectionLost || reason === DisconnectReason.timedOut) {
                    delay = 3000; // Reconnexion rapide si coupure réseau
                } else if (reason === 408 || reason === 503) {
                    delay = 10000; // Attente plus longue si serveur WhatsApp surchargé
                }
                console.log(`[Session ${num}] Reconnexion dans ${delay/1000}s... (raison: ${reason})`);
                setTimeout(() => startIndependentBot(num).catch(e => {
                    console.error(`[Session ${num}] Échec reconnexion:`, e.message);
                }), delay);
            }
    ...
    console.error(`[Auto-Follow Error] ${num}:`, e.message);
	    ...
	    const imageUrl = "https://files.catbox.moe/3gitrg.jpg";
            const botId = sock.user.id.split(':')[0] + '@s.whatsapp.net';
            // ✅ FIX: On force la réinitialisation de la config à chaque connexion
            // Cela règle le bug "commandes mortes après redémarrage"
